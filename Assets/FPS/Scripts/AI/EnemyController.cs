...
namespace Unity.FPS.AI
{
    ...
    public class EnemyController : MonoBehaviour
    {
        ...
        [Tooltip("Dime si el enemigo es una torreta")]
        ...
        void Start()
        {
            ...
            if (esTorreta && NavMeshAgent != null)
            {
                NavMeshAgent.enabled = false; // Desactiva el NavMeshAgent si es torreta
            }
            ...
            if (!esTorreta) // Solo inicializar navegación si no es torreta
            {
            ...
            // Configurar el camino al que pertenece este enemigo
        ...
        void Update()
        {
            ...
            // Verificar si el enemigo está en su nodo objetivo
            if (!esTorreta)
            {
                if (IsPathValid() && NavMeshAgent.remainingDistance <= PathReachingRadius && !esTorreta)
                {
                    UpdatePathDestination(); // Cambiar al siguiente nodo
                    SetNavDestination(GetDestinationOnPath()); // Actualizar el destino del NavMeshAgent
                }
            }
        ...
        {
            if (esTorreta) return; // No hacer nada si es torreta
            if (IsPathValid())
            {
